...
CREATE OR REPLACE FUNCTION public.delegates_forged_blocks_count_update()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$ BEGIN IF (TG_OP = 'INSERT') THEN UPDATE delegates SET blocks_forged_count = blocks_forged_count+1 WHERE public_key = NEW."generator_public_key"; ELSIF (TG_OP = 'DELETE') THEN UPDATE delegates SET blocks_forged_count = blocks_forged_count-1 WHERE public_key = OLD."generator_public_key"; END IF; RETURN NULL; END $function$
;
...
CREATE OR REPLACE FUNCTION public.get_delegates_list()
 RETURNS text[]
 LANGUAGE plpgsql
AS $function$ DECLARE list text[]; BEGIN SELECT generate_delegates_list( (SELECT CEIL((height+1) / 101::float)::int AS round FROM blocks ORDER BY height DESC LIMIT 1), ARRAY(SELECT ENCODE(public_key, 'hex') AS public_key FROM delegates ORDER BY rank ASC LIMIT 101) ) INTO list; RETURN list; END $function$
;
